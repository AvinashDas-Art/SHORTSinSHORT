    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0d0d0f]/90 backdrop-blur-md border-b border-zinc-800/60 px-4 md:px-8 py-3 flex items-center justify-between transition-all duration-300">
      ...
      {!isClubView ? (
        ...
        <div className="flex-1 max-w-xl mx-4 text-center">
          <span className="text-xs uppercase tracking-widest text-red-500 font-bold bg-red-950/40 px-3 py-1 rounded-full border border-red-800/40">
            ★ Exclusive Cinema Club
          </span>
        </div>
      )}
      ...
      <div className="flex items-center space-x-2.5 md:space-x-3">
        <button
          onClick={onOpenClub}
          className="text-xs font-bold px-3 py-1.5 rounded-full bg-gradient-to-r from-red-700 to-amber-600 hover:from-red-600 hover:to-amber-500 text-white shadow-md hover:shadow-amber-500/20 transition flex items-center space-x-1 cursor-pointer"
        >
          <span>★</span>
          <span>{isClubView ? (lang === 'hi' ? "गैलरी" : "Gallery") : (lang === 'hi' ? "क्लब ₹5" : "Club ₹5")}</span>
        </button>

        <button
          onClick={() => setLang(lang === 'en' ? 'hi' : 'en')}
          className="text-xs font-semibold px-2.5 py-1 rounded bg-zinc-900 border border-zinc-800 text-zinc-300 hover:text-white hover:border-zinc-700 transition cursor-pointer"
        >
          {lang === 'en' ? 'Hi' : 'En'}
        </button>
        ...
        {currentUser ? (
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl py-2 z-50">
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-2 text-xs text-zinc-300 hover:bg-zinc-800 transition cursor-pointer"
                >
                  ★ Open Cinema Club
                </button>
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-2 text-xs text-red-400 hover:bg-zinc-800 transition flex items-center space-x-2 cursor-pointer"
                >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold px-3 py-1.5 rounded-full shadow-md hover:shadow-red-600/30 transition transform hover:scale-105 cursor-pointer"
          >
